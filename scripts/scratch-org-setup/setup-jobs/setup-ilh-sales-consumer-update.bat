...
echo off
REM This script will install package version for ILH Sales Consumer Update package. 
REM  !!!!  This should only be used for scratch org setup.  !!!!!!!
REM
REM  Dependencies:   
REM       -  D2C Core Base Objects
REM       -  ILH Sales Base Objects
...
:Install
echo Getting latest package version id for Package Id: 0HoDm0000004CFGKA2  - ILH Sales Consumer Update
call sf data query --query "SELECT SubscriberPackageVersionId FROM Package2Version WHERE Package2ID = '0HoDm0000004CFGKA2' ORDER BY CreatedDate DESC LIMIT 1" > c:\temp\packageVerID.json --use-tooling-api --result-format json --target-org %devhub% --wait 2' 
...
echo **********  Installed Package ILH Sales Consumer Update  **********
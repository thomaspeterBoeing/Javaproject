...
:Install
echo Getting latest package version id for Package Id: 0HoDm0000004CFQKA2  - ILH Sales Rate Page
call sf data query --query "SELECT SubscriberPackageVersionId FROM Package2Version WHERE Package2ID = '0HoDm0000004CFQKA2' ORDER BY CreatedDate DESC LIMIT 1" > c:\temp\packageVerID.json --use-tooling-api --result-format json --target-org %devhub% --wait 2'  
...
echo **********  Installed Package ILH Sales Rate Page  **********

...
echo off
REM This script will install package version for ILH Sales Base code package. 
...
call sf data query --query "SELECT SubscriberPackageVersionId FROM Package2Version WHERE Package2ID = '0HoDm0000004CESKA2' AND IsDeprecated = False ORDER BY CreatedDate DESC LIMIT 1" > c:\temp\packageVerID.json --use-tooling-api --result-format json --target-org %devhub% --wait 2' 
...
echo **********  Installled Package ILH Sales Base Code ********** 

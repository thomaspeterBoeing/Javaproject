...
echo off
REM This script will install package version for ILH Sales Consumer Opportunity Automation. 
...
:Install
echo Getting latest package version id for Package Id: 0HoDm0000004CErKAM  - ILH Sales Consumer Opportunity Application
call sf data query --query "SELECT SubscriberPackageVersionId FROM Package2Version WHERE Package2ID = '0HoDm0000004CErKAM' AND IsDeprecated = False ORDER BY CreatedDate DESC LIMIT 1" > c:\temp\packageVerID.json --use-tooling-api --result-format json --target-org %devhub% --wait 2' 
...
echo **********  Installed Package ILH Sales Consumer Opportunity Automation  **********
